...
// 맞추고, 캔버스(1000x700) 밖으로 넘치는 경우에만 안쪽으로 클램프한다.
...
function RoomTooltip({ room, x, y }) {
    ...
    // 필요한 만큼의 높이만 사용한다.
    const height = Math.min(96 + lineCount * 25, VIEW_H - EDGE_MARGIN * 2);

    // 방의 중심(x, y)에 팝업 중심이 오도록 배치하되, 캔버스 밖으로 나가지 않도록 클램프한다.
    const boxX = Math.max(EDGE_MARGIN, Math.min(x - BOX_W / 2, VIEW_W - BOX_W - EDGE_MARGIN));
    const boxY = Math.max(EDGE_MARGIN, Math.min(y - height / 2, VIEW_H - height - EDGE_MARGIN));
